...
int deleteblock0(block B) {
    ...
    if (Btemp) {
        if (Btemp->pre) Btemp->pre->next = Btemp->next;
        else Bhead = Btemp->next;
    ...
    if (Pltemp->pre) Pltemp->pre->next = Pltemp->next;
    else Plhead = Pltemp->next;
    ...
    while (Prtemp) {
        ...
        if (ntemp) {
            if (Prtemp->n->n != c) {
                ...
                if (Prtemp->pre) Prtemp->pre->next = Prtemp->next;
