...
_3D OP = { 0.5, -0.5, 0.5 }; // 定义一个全局变量OP，表示光源
...
// 根据窗口大小和缩放比例计算光标位置，并打印星号    
void gotoxy(int x, int y, int c) {
    // 预先计算缩放后的窗口宽高(的一半)
    double scaledX = setwindowsize.X/2;  // 16.0 / 1.2
    double scaledY = setwindowsize.Y; // 16.0 / 1.4

    // 判断是否在可绘制区域
    if (x < scaledX  && -x < scaledX && y < scaledY  && -y < scaledY) {
        COORD pos;
